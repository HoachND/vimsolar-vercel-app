...
export default function AdminSidebar() {
  ...
  return (
    <aside className="w-64 bg-white border-r border-gray-200 h-screen overflow-y-auto flex-shrink-0 flex flex-col fixed left-0 top-0 pt-16 z-10 hidden md:flex">
      <div className="py-4">
        {menuGroups.map((group, idx) => (
          <div key={idx} className="mb-6">
            <h3 className="px-6 text-xs font-bold text-[#0070f3] uppercase tracking-wider mb-2">
              {group.title}
            </h3>
                  ...
                  <li key={item.href}>
                    <Link 
                      ...
                      className={`flex items-center gap-3 px-6 py-2.5 text-sm transition-colors ${
                        isActive 
                          ? "bg-[#0070f3]/10 text-[#0070f3] font-semibold border-r-4 border-[#0070f3]" 
                          : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"
                      }`}
                    >
                      <Icon size={18} className={isActive ? "text-[#0070f3]" : "text-gray-400"} />
                      {item.name}
                    </Link>
                  </li>
                );
